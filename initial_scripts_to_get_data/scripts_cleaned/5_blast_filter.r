date<-"24_June_2018"
bl<-read.table(file="working_dir/blast_for_edit_clean.txt",sep="\t",header=FALSE,fill=TRUE,na.strings = "", quote="\"")
...
#head(bl)
master<-read.csv(file="working_dir/MASTER_accession_list.csv",header=TRUE)
...
target.list<-master[which(master$ITSx_PASS=='PASS' & master$Quality_PASS=='PASS' & master$CladeBlast=='Trebouxia'),]
target.list.clade.a<-master[which(master$ITSx_PASS=='PASS' & master$Quality_PASS=='PASS' & master$SubCladeBlast=='Trebouxia_CladeA'),]
target.list.clade.i<-master[which(master$ITSx_PASS=='PASS' & master$Quality_PASS=='PASS' & master$SubCladeBlast=='Trebouxia_CladeI'),]
target.list.clade.g<-master[which(master$ITSx_PASS=='PASS' & master$Quality_PASS=='PASS' & master$SubCladeBlast=='Trebouxia_CladeG'),]
target.list.clade.s<-master[which(master$ITSx_PASS=='PASS' & master$Quality_PASS=='PASS' & master$SubCladeBlast=='Trebouxia_CladeS'),]
#nrow(target.list)
write(target.list$Organism_Acc_No,file="working_dir/target.taxa.txt")
write(target.list.clade.a$Organism_Acc_No,file="working_dir/target.taxa.A.txt")
write(target.list.clade.i$Organism_Acc_No,file="working_dir/target.taxa.I.txt")
write(target.list.clade.g$Organism_Acc_No,file="working_dir/target.taxa.G.txt")
write(target.list.clade.s$Organism_Acc_No,file="working_dir/target.taxa.S.txt")
