...
thresh_extract<-list.files("working_dir", pattern="^individual_[0-9,.]*._clusters", full.names=TRUE)
for(q in 1:length(thresh_extract)){
	working_thresh<-strsplit(thresh_extract[q],split="_")[[1]][3]
	repfile<-list.files(as.character(paste("working_dir/individual_",working_thresh,"_clusters",sep="")), pattern="^[0-9,.]*_rep_set_accessions.txt", full.names=TRUE)
