date<-"24_June_2018"
bl<-read.table(file="working_dir/passed_mothur.txt",sep="\t",header=FALSE,fill=TRUE,na.strings = "", quote="\"")
colnames(bl)<-"Taxon"
...
#nrow(target.list)
write(target.list$Organism_Acc_No,file="working_dir/target.taxa.passitsx.passmothur.txt")
